...
CELLS = [
    {
        "slug": "clawsum-platform",
        "name": "Clawsum Platform",
        "type": "platform",
        "description": "Empire control plane — Hermes, Paperclip, OpenClaw, Traefik, Postgres, clawsums@gmail.com ops",
        "systems_connected": ["paperclip", "hermes", "openclaw", "postgres", "gmail", "traefik", "grafana"],
        "allowed_reads": ["health", "approvals", "inbox_summaries", "task_board", "audit"],
        "allowed_actions": ["draft_runbooks", "summaries", "triage_inbox"],
        "approval_required": ["production_deploy", "credential_rotate", "enable_heartbeats", "dns_change"],
        "never_autonomous": ["wipe_db", "force_push_main", "billing_change", "disable_auth"],
        "daily_summary_fields": ["inbox_pending", "approvals", "blocked_tasks", "cell_health"],
        "primary_agent": "admin",
        "memory_namespace": "clawsum-platform",
    },
    {
        "slug": "personal-admin",
        "name": "Personal Admin",
        "type": "personal",
        "description": "Gerald personal — calendar, docs, ChatGPT archive personal scope; not client CRM",
        "systems_connected": ["gmail", "calendar", "obsidian", "chatgpt_archive"],
        "allowed_reads": ["schedule", "inbox_summaries", "task_history", "reminders"],
        "allowed_actions": ["draft_email", "draft_text", "meeting_summary", "create_reminder"],
        "approval_required": ["send_email", "send_text", "calendar_invite"],
        "never_autonomous": ["banking", "legal_filings", "payroll", "credential_changes"],
        "daily_summary_fields": ["inbox_highlights", "calendar", "reminders", "pending_approvals"],
        "primary_agent": "admin",
